...
#         await self._do_bootstrap()
# """
# scraper/platforms/noon/session_manager.py

# Owns the session lifecycle. Nothing in search.py / product.py should
# ever touch cookies, Scrapfly, or curl_cffi Session objects directly —
# they call SessionManager and get back what they need.

# DESIGN CHANGE FROM THE OLD VERSION (deliberate, tested):
#     Old: time-based health_check() guessing at JWT/cookie TTLs, a
#          separate whoami call to mint nguestv2, a locked proxy_url
#          baked into the bundle at bootstrap time.
#     New: reactive only. We don't track any expiry clock ourselves.
#          A live API call is the only source of truth on whether the
#          current cookie jar still works. If it works, keep using it.
#          If the scraper reports a block, bootstrap fresh via Scrapfly
#          (browser.py) and hand back a new jar. nguestv2 isn't tracked
#          at all — it rides along automatically inside the persistent
#          curl_cffi Session's own cookie jar, refreshed for free by
#          noon's API on every call via Set-Cookie.

# WHY ONE LONG-LIVED curl_cffi SESSION:
#     curl_cffi's Session object keeps its own cookie jar and merges in
#     every Set-Cookie it receives, exactly like a browser tab does.
#     Reusing one Session for the entire run means nguestv2 rotation is
#     handled by curl_cffi itself — we never read, decode, or write that
#     cookie ourselves. Recreating a Session per call would throw that
#     jar away and lose any cookie noon updated since bootstrap.

# PROXY:
#     Off by default (USE_PROXY = False). The plan is to run direct IP
#     until we actually observe blocks, then flip the flag — not to
#     assume proxy is required before we've measured that it is.

# BOOTSTRAP LOG:
#     We don't persist the cookie jar itself to disk (no reason to — a
#     fresh bootstrap is cheap and a stale jar on disk is just a trap
#     for a future run to load something already-dead). What we DO log,
#     one line per event, is timestamps: every bootstrap and every
#     detected failure. That log is what eventually answers "how long
#     do these cookies actually live" — not a hardcoded constant.
# """
...
# from app.config import settings
# from scraper.platforms.noon.browser import bootstrap_session, BootstrapError
...
#     getattr(settings, "BOOTSTRAP_LOG_PATH", "backend/scraper/platforms/noon/data/bootstrap_log.csv")
# )

# # Where the cookie jar itself gets persisted between process runs.
# # Unlike the old design, we DO save actual cookie values here now —
# # the only cost of a stale file is one wasted probe request, and the
# # upside is skipping a Scrapfly call entirely when cookies are still good.
# COOKIE_STATE_PATH = Path(
#     getattr(settings, "SESSION_COOKIES_PATH", "backend/scraper/platforms/noon/data/session_cookies.json")
# )

# # Cheap request used only to check "do these cookies still work" —
# # not a real scrape, just a canary. Kept local to this file (rather
# # than imported from search.py) to avoid a circular import between
# # session_manager.py and search.py.
# CANARY_URL = (
#     "https://www.noon.com/_vs/nc/mp-customer-catalog-api/api/v3/u/search/"
#     "?q=phone&page=1"
# )
# CANARY_REFERER = "https://www.noon.com/uae-en/search/?q=phone"

# # Cookies that belong to third-party domains, not noon.com — a real
# # browser never forwards these to noon's API, so we don't either.
# THIRD_PARTY_COOKIES = frozenset({
#     "bcookie", "lidc", "bh",
#     "IDE",
#     "CLID",
...
# # ─────────────────────────────────────────────────────────────────────────────
# # SESSION BUNDLE (lightweight — no JWT, no x_headers, no locked proxy)
# # ─────────────────────────────────────────────────────────────────────────────

# @dataclass
# class SessionBundle:
#     """
#     Just enough state for header-building; cookie state itself lives
#     in the curl_cffi Session, not here.

#     sec_ch_ua / sec_ch_ua_mobile / sec_ch_ua_platform / accept_language
#     are copied verbatim from whatever Scrapfly's browser reported for
#     THIS bootstrap — never hardcoded defaults. See browser.py.
#     """
#     user_agent:         str
#     bootstrap_time:     datetime
#     sec_ch_ua:          str
...
# def _save_cookie_state(raw: dict) -> None:
#     """
#     Persists the full bootstrap_session() output to disk — cookies plus
#     every client-identity field (sec-ch-ua, platform, accept-language,
#     chrome_version) — so the next process run can try reusing the exact
#     same identity, not just the cookies on their own. Reusing cookies
#     with a DIFFERENT platform/UA than the one that earned them would
#     reintroduce the mismatch this whole change was meant to fix.
#     """
#     try:
#         COOKIE_STATE_PATH.parent.mkdir(parents=True, exist_ok=True)
#         payload = dict(raw)
#         payload["bootstrap_time"] = raw["bootstrap_time"].isoformat()
#         COOKIE_STATE_PATH.write_text(json.dumps(payload, indent=2), encoding="utf-8")
#     except Exception as exc:
#         logger.warning(f"[SessionManager] Failed to save cookie state: {exc}")


# def _load_cookie_state() -> Optional[dict]:
#     """
#     Loads a previously saved cookie jar from disk, if present.
#     Returns None on missing file, corrupt JSON, or any read error —
#     caller treats None exactly like "no saved state, bootstrap fresh".
#     Does NOT validate the cookies still work — that's what the probe
#     request in initialise() is for. This function only answers
#     "is there something worth trying".
#     """
#     if not COOKIE_STATE_PATH.exists():
#         return None
#     try:
#         payload = json.loads(COOKIE_STATE_PATH.read_text(encoding="utf-8"))
#         if "bm_sv" not in payload.get("cookies", {}):
#             logger.info("[SessionManager] Saved cookie state missing bm_sv. Ignoring.")
#             return None
...
#         if missing:
#             logger.info(
#                 f"[SessionManager] Saved cookie state missing {missing} "
#                 f"(likely from an older format). Ignoring."
#             )
#             return None
#         return payload
#     except Exception as exc:
#         logger.warning(f"[SessionManager] Saved cookie state unreadable: {exc}. Ignoring.")
#         return None


# def _log_event(event: str, detail: str = "") -> None:
#     """
#     Appends one line: ISO timestamp, event type, optional detail.
#     event is one of: "bootstrap", "block_detected".
#     This is the raw material for figuring out real cookie lifetime —
#     diff consecutive bootstrap/block_detected timestamps across runs.
#     """
#     try:
...
#             f.write(line)
#     except Exception as exc:
#         # Logging failure should never take down a scrape run.
#         logger.warning(f"[SessionManager] Failed to write bootstrap log: {exc}")
...
# def _pick_impersonate_target(chrome_version: Optional[int]) -> str:
#     """
#     Picks the closest curl_cffi-supported Chrome target to whatever
#     version this bootstrap's UA actually reported. Falls back to the
#     newest supported version only if the harvest genuinely couldn't
#     parse a version at all (see browser.py's warning for that case).
#     """
#     if chrome_version is None:
#         fallback = SUPPORTED_IMPERSONATE_CHROME_VERSIONS[-1]
#         logger.warning(
#             f"[SessionManager] No chrome_version from bootstrap. "
#             f"Falling back to chrome{fallback}."
#         )
#         return f"chrome{fallback}"

#     closest = min(
#         SUPPORTED_IMPERSONATE_CHROME_VERSIONS,
#         key=lambda v: abs(v - chrome_version),
#     )
#     if closest != chrome_version:
#         logger.info(
#             f"[SessionManager] curl_cffi has no exact chrome{chrome_version} "
#             f"target. Using closest available: chrome{closest}."
#         )
#     return f"chrome{closest}"


# def _build_headers(bundle: "SessionBundle", referer: str) -> dict[str, str]:
#     """
#     Everything EXCEPT cookies — the curl_cffi Session attaches cookies
#     itself from its own jar. Passing a manual 'cookie' header here
#     would fight with that and is deliberately omitted.

#     Every client-identity value (sec-ch-ua, platform, accept-language)
#     comes from the bundle — i.e. from what Scrapfly's browser actually
#     sent during THIS bootstrap — not from a fixed string. See browser.py
...
#         "priority":           "u=1, i",
#         "referer":            referer,
#         "sec-ch-ua":          bundle.sec_ch_ua,
#         "sec-ch-ua-mobile":   bundle.sec_ch_ua_mobile,
#         "sec-ch-ua-platform": bundle.sec_ch_ua_platform,
#         "sec-fetch-dest":     "empty",
#         "sec-fetch-mode":     "cors",
#         "sec-fetch-site":     "same-origin",
#         "user-agent":         bundle.user_agent,
...
# class SessionManager:
#     """
#     Public surface for search.py / product.py:

#         await session_manager.initialise()
#         session = session_manager.get_session()      # curl_cffi Session
#         headers = session_manager.get_headers(referer)
#         proxy   = session_manager.get_proxy()         # None unless USE_PROXY
#         session_manager.log_request()
#         await session_manager.handle_block()          # on 403 / confirmed block

#     No health_check(), no ensure_valid() polling a clock — bootstrap
#     happens once at initialise(), and again only when handle_block()
#     is explicitly called by the scraper after it observes a failure.
#     """
...
#         country: str = "ae",
#     ) -> None:
#         self._api_key:   str            = scrapfly_api_key or settings.SCRAPFLY_API_KEY
...
#             raise ValueError(
#                 "[SessionManager] use_proxy=True but no proxy_url provided."
#             )
...
#     async def initialise(self) -> None:
#         """
#         Entry point. Call once at startup before any scraping.

#         Tries the saved cookie jar first — no harm in it, and it's
#         strictly cheaper than a Scrapfly call:
#           1. Load cookies from disk, if any.
#           2. Seed a Session with them.
#           3. Fire one canary probe request.
#           4. If the probe succeeds → done, no bootstrap needed.
#           5. If there's nothing saved, or the probe fails → bootstrap
#              fresh via Scrapfly, exactly like before.
#         """
#         saved = _load_cookie_state()

#         if saved is not None:
#             logger.info("[SessionManager] Found saved cookie state. Probing before bootstrap...")
#             self._load_bundle_and_session(saved)

#             if await self._probe_session():
#                 logger.info("[SessionManager] Saved cookies still valid. Skipping bootstrap.")
#                 return

#             logger.info("[SessionManager] Saved cookies failed probe. Bootstrapping fresh.")

#         await self._do_bootstrap()

#     def get_session(self) -> Session:
#         """
#         Returns the single long-lived curl_cffi Session for this run.
#         search.py / product.py call session.get(url, headers=..., proxy=...)
#         directly on this — cookies persist and update automatically
#         across every call for the lifetime of the run.
#         """
#         if self._session is None:
#             raise RuntimeError(
#                 "[SessionManager] get_session() called before initialise()."
#             )
#         return self._session

#     def get_headers(self, referer: str) -> dict[str, str]:
#         """Non-cookie headers for a request. See _build_headers()."""
#         if self._bundle is None:
#             raise RuntimeError(
#                 "[SessionManager] get_headers() called before initialise()."
#             )
#         return _build_headers(self._bundle, referer)

#     def get_proxy(self) -> Optional[str]:
#         """
#         Returns the proxy URL if USE_PROXY is on, else None.
#         curl_cffi treats proxy=None as "no proxy" — safe to pass
...
#     async def handle_block(self, reason: str = "") -> None:
#         """
#         Call this when the scraper confirms a block:
#           - HTTP 403 from noon's API, or
#           - two consecutive 200-with-empty-hits responses on a
#             keyword/product that should have results

#         Logs the failure event, then re-bootstraps via Scrapfly and
#         replaces the Session with a fresh one (old cookies discarded
#         entirely rather than merged — a blocked jar isn't worth
#         salvaging any part of).
#         """
#         logger.warning(f"[SessionManager] Block detected. Reason: {reason}")
#         _log_event("block_detected", reason)
#         await self._do_bootstrap()
...
#         now = datetime.now(timezone.utc)
#         age_s = (now - self._bundle.bootstrap_time).total_seconds()

#         return {
#             "status":            "active",
#             "bootstrap_age_s":   round(age_s, 1),
#             "request_count":     self._bundle.request_count,
...
#     def _load_bundle_and_session(self, saved: dict) -> None:
#         """
#         Builds self._bundle / self._session from a dict shaped like
#         what _load_cookie_state() or bootstrap_session() produces.
#         Shared by both the "loaded from disk" and "fresh bootstrap"
#         paths so cookie-seeding logic only lives in one place.

#         Every identity field (UA, sec-ch-ua, platform, accept-language,
#         impersonate target) is taken from `saved` — i.e. from whatever
#         Scrapfly's browser actually reported for that bootstrap. Nothing
#         here falls back to a hardcoded default.
#         """
#         bootstrap_time = saved["bootstrap_time"]
#         if isinstance(bootstrap_time, str):
#             bootstrap_time = datetime.fromisoformat(bootstrap_time)

#         impersonate_target = _pick_impersonate_target(saved.get("chrome_version"))

#         self._bundle = SessionBundle(
#             user_agent=saved["user_agent"],
#             bootstrap_time=bootstrap_time,
#             sec_ch_ua=saved["sec_ch_ua"],
#             sec_ch_ua_mobile=saved["sec_ch_ua_mobile"],
#             sec_ch_ua_platform=saved["sec_ch_ua_platform"],
#             accept_language=saved["accept_language"],
...
#             if name in THIRD_PARTY_COOKIES:
#                 continue
#             self._session.cookies.set(name, value, domain=".noon.com")

#     async def _probe_session(self) -> bool:
#         """
#         Fires one cheap request against the search API to check whether
#         the current session's cookies still work. Returns True only on
#         a clean 200 with nbHits > 0 — anything else (403, network error,
#         malformed JSON, nbHits == 0) is treated as "cookies are dead,
#         don't trust this jar."
#         """
#         try:
#             resp = await self._session.get(
#                 CANARY_URL,
#                 headers=_build_headers(self._bundle, CANARY_REFERER),
#                 proxy=self.get_proxy(),
#                 timeout=15,
#             )
#         except Exception as exc:
#             logger.info(f"[SessionManager] Probe request failed: {exc}")
#             return False

#         if resp.status_code != 200:
#             logger.info(f"[SessionManager] Probe got HTTP {resp.status_code}.")
#             return False

#         try:
#             data = resp.json()
#         except Exception:
#             logger.info("[SessionManager] Probe response wasn't valid JSON.")
#             return False

#         nb_hits = data.get("nbHits", 0)
#         if nb_hits <= 0:
#             logger.info(f"[SessionManager] Probe returned nbHits={nb_hits}.")
#             return False
...
#     async def _do_bootstrap(self) -> None:
#         """
#         Runs Scrapfly bootstrap (sync SDK call, offloaded to a thread
#         so it doesn't block the event loop), builds a fresh Session
#         seeded with the harvested cookies, and saves that jar to disk
#         so the next process run can try reusing it.
#         """
#         logger.info("[SessionManager] Bootstrapping via Scrapfly...")
...
#             logger.error(f"[SessionManager] Bootstrap failed: {exc}")
#             raise

#         # Fresh Session every bootstrap — old jar is discarded, not merged.
#         self._load_bundle_and_session(raw)

#         _save_cookie_state(raw)
#         _log_event(
#             "bootstrap",
#             f"cookies={len(raw['cookies'])} platform={raw['sec_ch_ua_platform']} "
#             f"chrome={raw['chrome_version']}",
#         )

#         logger.info(
#             f"[SessionManager] Bootstrap complete. "
#             f"Cookies loaded: {len(raw['cookies'])}. "
#             f"UA: {raw['user_agent'][:60]}... "
#             f"Impersonate: {self._bundle.impersonate_target}."
#         )
...
WHY ONE LONG-LIVED curl_cffi SESSION:
    curl_cffi's AsyncSession object keeps its own cookie jar and merges
    in every Set-Cookie it receives, exactly like a browser tab does.
    Reusing one Session for the entire run means nguestv2 rotation is
    handled by curl_cffi itself — we never read, decode, or write that
    cookie ourselves. Recreating a Session per call would throw that
    jar away and lose any cookie noon updated since bootstrap.
...
COOKIE PERSISTENCE + BOOTSTRAP LOG:
    The full bootstrap_session() output (cookies AND every
    client-identity header field — sec-ch-ua, platform, accept-language,
    chrome_version) is saved to disk after every bootstrap, so the next
    process run can try reusing the exact same identity before paying
    for a fresh Scrapfly call. Separately, a lightweight append-only
    CSV log records just two event types with timestamps — "bootstrap"
    and "block_detected" — which is the raw material for eventually
    answering "how long do these cookies actually live", empirically,
    rather than trusting a hardcoded TTL.

CURL_CFFI IMPERSONATE TARGET — WHY IT CAN'T JUST BE THE REAL CHROME VERSION:
    sec-ch-ua / User-Agent are just HTTP header text — Scrapfly's real
    Chrome version (e.g. 152) goes in there directly, dynamically, no
    guessing involved.
    curl_cffi's `impersonate=` parameter is a different thing entirely:
    it selects a pre-compiled TLS ClientHello fingerprint baked into
    the library's binary at build time. curl_cffi cannot fingerprint a
    Chrome version it hasn't shipped a target for — and no library's
    release cadence keeps up with Chrome's, so the real version and the
    closest available TLS target will almost always differ. We ask
    curl_cffi itself (via curl_cffi.const.BrowserType) what it actually
    ships in THIS install, rather than hand-typing a version list from
    memory of the docs — that mismatch is exactly what caused the
    "Impersonating chrome133 is not supported" failure.
"""
...
BOOTSTRAP_LOG_PATH = Path(
    getattr(settings, "BOOTSTRAP_LOG_PATH", "data/bootstrap_log.csv")
)

# Where the full bootstrap output (cookies + identity headers) gets
# persisted between process runs.
COOKIE_STATE_PATH = Path(
    getattr(settings, "SESSION_COOKIES_PATH", "data/session_cookies.json")
)
...
def _get_supported_chrome_versions() -> list[int]:
    """
    Asks curl_cffi itself which chromeNNN impersonate targets exist in
    THIS installed version — not a list typed from memory of the docs.
    That's exactly what bit us: chrome133 was in a hand-written list
    but wasn't actually shipped in the install being tested against,
    and curl_cffi rejected it outright at request time instead of at
    pick-time.

    curl_cffi exposes its supported targets via curl_cffi.const.BrowserType
    (an enum of every impersonate string it ships, across all browsers,
    not just Chrome). We filter down to the "chromeNNN" ones and parse
    out the version number. Result is cached at module import — this
    ...
    try:
        ...
        for member in BrowserType:
            name = member.value if hasattr(member, "value") else str(member)
            match = re.fullmatch(r"chrome(\d+)", name)
            if match:
        ...
        logger.warning(
            f"[SessionManager] Could not introspect curl_cffi.const.BrowserType: "
            f"{exc}. Falling back to a conservative single target."
        )

    # If introspection genuinely fails (unexpected curl_cffi API change),
    # fall back to whatever we can actually confirm works from your own
    # last successful test run, rather than guessing a version number.
    return [131]


SUPPORTED_IMPERSONATE_CHROME_VERSIONS = _get_supported_chrome_versions()
logger.info(
    f"[SessionManager] curl_cffi supports chrome targets: "
    f"{SUPPORTED_IMPERSONATE_CHROME_VERSIONS}"
)
...
def _save_cookie_state(raw: dict) -> None:
    ...
    same identity, not just the cookies on their own. Reusing cookies
    with a DIFFERENT platform/UA than the one that earned them would
    reintroduce the mismatch this whole design is meant to avoid.
    """
    try:
        COOKIE_STATE_PATH.parent.mkdir(parents=True, exist_ok=True)
        payload = dict(raw)
...
def _load_cookie_state() -> Optional[dict]:
    """
    Loads a previously saved cookie jar + identity fields from disk,
    if present. Returns None on missing file, corrupt JSON, an older
    save format missing the identity fields, or any read error —
    caller treats None exactly like "no saved state, bootstrap fresh".
...
class SessionManager:
    ...
    Public surface for search.py / product.py:
        ...
        await session_manager.initialise()
        session = session_manager.get_session()      # curl_cffi AsyncSession
        headers = session_manager.get_headers(referer)
        proxy   = session_manager.get_proxy()         # None unless USE_PROXY
        session_manager.log_request()
        await session_manager.handle_block()          # on 403 / confirmed block
    ...
    async def initialise(self) -> None:
        ...
        Tries the saved cookie jar first — no harm in it, and it's
        strictly cheaper than a Scrapfly call:
          1. Load cookies + identity headers from disk, if any.
          2. Seed a Session with them.
          3. Fire one canary probe request.
          4. If the probe succeeds → done, no bootstrap needed.
          5. If there's nothing saved, or the probe fails → bootstrap
    ...
    def get_session(self) -> AsyncSession:
        """
        Returns the single long-lived curl_cffi AsyncSession for this run.
        search.py / product.py call `await session.get(url, headers=..., proxy=...)`
        directly on this — cookies persist and update automatically
        across every call for the lifetime of the run.
        """
        if self._session is None:
    ...
    async def _do_bootstrap(self) -> None:
        ...
        Runs Scrapfly bootstrap (sync SDK call, offloaded to a thread
        so it doesn't block the event loop), builds a fresh Session
        seeded with the harvested cookies, and saves that jar + identity
        fields to disk so the next process run can try reusing them.
        """
        logger.info("[SessionManager] Bootstrapping via Scrapfly...")
