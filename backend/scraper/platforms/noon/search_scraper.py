...
#     return all_products
"""
scraper/platforms/noon/search.py

Scrapes Noon search results using the persistent curl_cffi Session
owned by SessionManager.

WHAT CHANGED FROM THE OLD search_scraper.py:
  - No more session_manager.ensure_valid() before every page. There's
    no time-based clock to check anymore — SessionManager already
    validated (or bootstrapped) its session once at initialise(), and
    stays on that session until handle_block() says otherwise.
  - No more `async with AsyncSession(...) as curl:` per request. We
    now call session_manager.get_session() once and reuse the same
    curl_cffi Session object for every page — that's what lets cookies
    (including nguestv2, refreshed automatically via Set-Cookie) carry
    forward across calls with zero manual bookkeeping.
  - session_manager.handle_block() calls now carry a `reason` string,
    since the new session_manager logs that reason to the bootstrap
    event log (useful later for "how often are we getting blocked and
    why" analysis).

WHAT DID NOT CHANGE:
  extract_product(), SORT_OPTIONS, dedup logic, pagination logic,
  consecutive-empty-page detection — all identical to before.
"""
...
async def scrape_page(
    ...
    """
    Scrapes a single search results page via the persistent Session.

    Flow:
      1. Build URLs
      2. Get session / headers / proxy from SessionManager
      3. GET on the persistent session (no ensure_valid() — the
         session is assumed good until handle_block() says otherwise)
      4. Response handling — 200/403/error cases
      5. Extract products
      6. log_request()
    """
    api_url = build_api_url(keyword, page_number, sort_by)
    ...
    if resp.status_code == 403:
        ...
        await session_manager.handle_block(reason=f"403 on search:{keyword} page={page_number}")
        return [], None
...
async def scrape_search(
    ...
    """
    Scrapes multiple pages of Noon search results for a keyword.
    Logic unchanged from before — dedup, pagination, consecutive-empty
    detection all identical. Only the session plumbing underneath it
    ...
    for page_number in range(1, pages + 1):
        ...
        if not products:
            ...
            if consecutive_empty >= 2:
                ...
                await session_manager.handle_block(
                    reason=f"two consecutive empty pages on search:{keyword}"
                )
                break
        else:
            consecutive_empty = 0
