...
async def bootstrap_session(proxy_dict: dict, proxy_url: str) -> dict:
    ...
    try:
        ...
        # ── Steps 6 & 7: Poll for bm_sv while running human simulation ────────
        logger.info("[Bootstrap] Polling for bm_sv while running human simulation...")

        async def _poll_for_bm_sv() -> bool:
            for attempt in range(10):
                ...
                try:
                    ...
                    names   = {c["name"] for c in current}
                    if "bm_sv" in names:
                        logger.info(f"[Bootstrap] bm_sv written after {(attempt+1)*3}s.")
                        return True
                    logger.debug(f"[Bootstrap] bm_sv not yet present ({(attempt+1)*3}s elapsed).")
                except Exception:
                    pass
            logger.warning("[Bootstrap] bm_sv not written after 30s polling.")
            return False

        # Run both concurrently — sensor gets the mouse events it needs
        bm_sv_found, _ = await asyncio.gather(
            _poll_for_bm_sv(),
            _human_simulation(page),
        )

        # Run a second human simulation during the remaining sensor wait
        # bm_sv polling takes up to 30s; human sim takes ~4-6s
        # This fills the gap so mouse activity is present throughout
        if not bm_sv_found:
            await _human_simulation(page)
            await asyncio.sleep(3.0)
